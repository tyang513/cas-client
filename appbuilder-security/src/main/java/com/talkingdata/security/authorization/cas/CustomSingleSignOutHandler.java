...
public final class CustomSingleSignOutHandler {
    ...
    private String uncompressLogoutMessage(final String originalMessage) {
        final byte[] binaryMessage = Base64.decodeBase64(originalMessage.getBytes(StandardCharsets.UTF_8));
        ...
        try {
            ...
            return new String(result, 0, resultLength, StandardCharsets.UTF_8.name());
        } catch (final Exception e) {
    ...
    private String computeRedirectionToServer(final HttpServletRequest request) {
        ...
        if (StringUtils.isNotBlank(relayStateValue)) {
            ...
            char sign = '/';
            if (!this.casServerUrlPrefix.endsWith(sign + "")) {
                buffer.append(sign);
            }
            buffer.append("logout?_eventId=next&");
            buffer.append(this.relayStateParameterName);
            buffer.append('=');
            buffer.append(urlEncode(relayStateValue));
